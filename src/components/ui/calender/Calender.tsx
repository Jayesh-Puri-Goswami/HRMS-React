...
export function Calendar({ onDateSelect }: CalendarProps) {
  ...
  return (
    <div className="p-4 bg-white dark:bg-white/[0.03] rounded-3xl shadow-sm border border-gray-200 dark:border-white/10 w-full max-w-full lg:max-w-2xl mx-auto">
      ...
      <div className="grid grid-cols-7 sm:grid-cols-7 gap-2 min-[375px]:gap-1">
        {calendarDays.map((day, index) => (
          <div
            ...
            className={`relative aspect-square min-h-[0px] md:min-h-[70px] p-2 rounded-xl border cursor-pointer transition-all duration-200 ${
          ...
          >
            <div className="flex flex-col items-start justify-between h-full">
              {day.specialLabel ? (
                <div className="text-2xl font-semibold text-indigo-500 dark:text-indigo-400 bg-indigo-100 dark:bg-indigo-900/30 px-2 py-1 rounded-md">
                  {day.specialLabel}
                </div>
              ) : (
                <span
